...
async function markAllMessagesRead(event) {
  ...
  if (auth.error) return auth.error
  const userId = auth.user.user_id || auth.user._id
  // 服务端 where().update() 批量更新全部可见消息；按用户记录已读，不影响同一门店其他成员
  await db.collection('message')
    .where(messageVisibilityWhere(auth.user))
    .update({ data: { read_by: _.push(userId), read_at: db.serverDate() } })
  return { code: 0 }
}
...
async function getHomeStats(event) {
  ...
  const [pendingApprovalRes, pendingReceiveRes, completedRes, attentionRes, ordersRes, reportsRes] = await Promise.all([
    ...
    db.collection('message').where({ ...messageVisibilityWhere(user), read: false, read_by: _.neq(user.user_id || user._id || '') }).count(),
